...
def get_cplex_sol(lp_file: str, obj_fn):
    ...
    print("Solving with classical CPLEX (60-second time limit)...")
    sol = model.solve()
    print("Classical solver finished.")
    if sol is None:
        raise RuntimeError("Classical solver could not find a solution within the time limit.")
...
def get_backend(device: str, instance: str, num_vars: int) -> BackendV2:
    ...
    if device == 'AerSimulator':
        ...
        print("✅ AerSimulator configured to use GPU.")
        return simulator
